...
export function BidSlider({ value, min, max, appraisedValue, onChange }: Props) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
      ...
      <div className="flex justify-between text-xs text-slate-400 dark:text-slate-500 mt-2">
        <span>최저 {formatPrice(min * 10000)}</span>
        <span>감정가 {formatPrice(appraisedValue * 10000)}</span>
      </div>
