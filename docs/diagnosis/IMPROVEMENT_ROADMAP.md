...
- [x] (2026-07-12) 코드 수리 완료 — 실행 중 발견: 틸코 연동이 스펙 불일치로 한 번도 성공한 적 없었음.
  ① 주소검색이 평문 바디 요구 (암호화 바디 → 7701001 실패), ② 응답 XML은 XmlData 필드, ③ 검색 결과 주소에 HTML 태그 혼입.
  → `tilko_provider.py` 수리 완료 (커밋 6abe5f6). 주소검색→고유번호 실동작 확인. 포인트 잔액 9,880pt.
- [ ] **블로커 (사용자 액션):** 인터넷등기소 로그인 실패 — 2025-01 개편 후 **회원 ID 로그인만 지원**.
  서버 `.env`의 `IROS_PHONE_NO`(=회원 아이디)/`IROS_PASSWORD` 를 유효한 등기소 회원 계정으로 갱신 후
  `bulk_registry_analysis.py --max 50` 재실행 → legal_score 회복 검증
...
- [x] (2026-07-12) 코드/유닛 완료 (커밋 4a902ca): SIGTERM 우아 종료 + 법원별 실패 격리 + 서울/경기 서비스 분할(deploy/).
  stuck 54건 DB 정리(잔여 0), 7/12 새벽 실패분(대법원 점검 시간대 충돌) 수동 백필 2,511건 완료.
  추가 발견: 서버 설치 유닛이 구버전(7200s)이었음 — deploy/ 파일과 불일치가 stuck 재발의 직접 원인.
- [ ] **사용자 액션 (sudo):** 새 유닛 설치 — 최종 보고의 명령어 참조. 이후 7일 stuck 0건 확인
...
**완료 기준:** 기본값 시크릿으로 서명된 토큰이 검증 실패.
- [x] (2026-07-14) 완료 (커밋 66ef030): JWT 기본값/빈값 거부, 웹훅 secret 미설정 시 503, .env.example 보완.
- [ ] **사용자 액션:** 서버 `TELEGRAM_WEBHOOK_SECRET` 설정 + setWebhook 재호출 (웹훅 쓸 경우), `sudo systemctl restart kyungsa`
...
- [x] (2026-07-14) 완료 (커밋 115255e): SignalBadge 신규, 홈/즐겨찾기/비교 신호등 통일, 비교 ML 행 제거, GradeLegend 개편. 빌드 통과.
...
- [x] (2026-07-14) 완료 (커밋 c0340a6): 직접 확인 항목을 📋 체크리스트 스타일로 분리, legal_score 보유 시 선순위권리 실신호 전환.
...
**작업 순서:** 삭제 전 각 파일 역참조 grep 재확인 → 테스트 + `npm run build` 통과 → 커밋 분리(프론트/백엔드/루트).
**완료 기준:** 테스트/빌드 통과, README 구조도 갱신.
- [x] (2026-07-17) 완료 (커밋 2b4538c): 39파일 -2,484줄. 프론트 9종 + v0 API + frontend_backup/ + 루트 스텁.
  CODEF 경로는 레거시 백업으로 보존 (사용자 결정 대기). 테스트 727/730 통과 (763→730은 죽은 v0 테스트 33개 제거분).
...
- [x] (2026-07-17) 완료: 틸코 전환 반영(구조도/데이터 흐름/2단 파이프라인), 테스트 수 730, v0 제거 반영, .env.example 보완.
  잔여 CODEF 언급은 변경 이력·레거시 백업 설명만.
...
| 2026-07-12 | W1 착수 → 틸코 스펙 드리프트 발견·수리 (6abe5f6) | 주소검색 성공, IROS 회원 계정 블로커로 중단 |
| 2026-07-12 | W2 완료 (4a902ca) — stuck 54건 정리 + 백필 2,511건 | 유닛 설치(sudo)만 사용자 대기 |
| 2026-07-14 | W3 (66ef030) + W4 (115255e) + W5 (c0340a6) | 테스트/빌드 통과 |
| 2026-07-17 | W6 (2b4538c) + W7 문서 정합화 | 다음: W8(사용자 결정) → W9 ML 재학습 |
