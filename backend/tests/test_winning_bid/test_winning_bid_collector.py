"""WinningBidCollector 단위 테스트 (Phase 6.5)
...
def _setup(db_session, auction: Auction, score: Score):
...
class TestWinningBidCollectorBasic:
    def test_basic_update(self, db_session):
        """낙찰 물건 1건 → actual_winning_bid/ratio/error 업데이트"""
...
class TestFiltering:
    ...
    def test_skip_already_updated_auction(self, db_session):
        """actual_winning_bid 이미 있으면 조회 대상 제외"""
        auction = _make_auction()
        # actual_winning_bid 이미 설정
        ...
        # 조회 자체가 안됨 (WHERE actual_winning_bid IS NULL)
